...
% ============================================================
\begin{frame}{Przegląd literatury: Empiryczny wpływ heterogeniczności (Zhao et al.)}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Ilościowy dowód degradacji FedAvg}\\[4pt]
Zhao et al. (2018) empirycznie wykazali negatywny wpływ heterogeniczności danych na FedAvg, demonstrując wyraźny spadek dokładności w warunkach non-IID.\\[6pt]
Rozmiar tej różnicy w wydajności powiększa się w miarę wzrostu stopnia heterogeniczności danych klientów.
\end{column}
\begin{column}{0.5\textwidth}
\begin{infoboxgreen}{Wniosek komunikacyjny}
Analizy teoretyczne ugruntowały fakt, że heterogeniczność danych zwiększa liczbę rund komunikacyjnych potrzebnych do osiągnięcia założonej dokładności, dodatkowo pogłębiając wąskie gardło komunikacyjne w FL.
\end{infoboxgreen}
\end{column}
\end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{Przegląd literatury: Mapowania empiryczne (Li et al.)}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Odwzorowania heterogeniczność--strategia}\\[4pt]
Li et al. symulują zróżnicowane warunki non-IID i oceniają strategie agregacji w tych warunkach, tworząc empiryczne mapowania między poziomem heterogeniczności a wydajnością strategii.\\[6pt]
Podobne podejście empiryczne zaprezentował Dubey (2025), który skwantyfikował heterogeniczność jako metrykę i rekomendował strategie sprawdzone w porównywalnych warunkach.
\end{column}
\begin{column}{0.5\textwidth}
\begin{infobox}{Kluczowe ograniczenie}
Większość tych prac zwraca \textbf{tylko nazwy strategii}, bez strojenia ich parametrów (np. $\mu$ w FedProx czy $f$ w Krum), mimo że parametry te mają istotny wpływ na wydajność.\\[6pt]
Sztywne tabele mapowań nie adaptują się do nowych algorytmów agregacji i mogą nie generalizować się na zbiory danych o odmiennej charakterystyce.
\end{infobox}
\end{column}
\end{columns}
\end{frame}
...
\begin{tabular}{p{2.1cm}p{1.75cm}p{1.95cm}p{1.55cm}p{1.75cm}p{1.85cm}}
\toprule
...
Mawela et al. (2025) & Nie & Nie & Nie & Tak & Nie\\
Shen et al. (2023) & Nie & Nie & Nie & Tak & Nie\\
...
\begin{itemize}\footnotesize
\item \textbf{Wbudowana fl\_schema:} Model otrzymuje dokładną listę dozwolonych strategii i przedziałów liczbowych parametrów, co zwiększyło poprawność wyjścia z 2 na 5 do 5 na 5 przypadków testowych.
\item \textbf{Few-Shot Prompting:} Umieszczenie konkretnych przykładów poprawnego kodu JSON w instrukcji systemowej; bez niego błędy występowały w 2 z 5 przypadków testowych.
\end{itemize}
\end{infobox}
\end{column}
\end{columns}
...
\textbf{Krok 3:} Przy braku poprawy po 3 próbach, system zgłasza błąd informacyjny (nie uruchamia automatycznie żadnej domyślnej strategii).
\end{infoboxgreen}
\end{column}
\end{columns}
\end{frame}
...
\item Feature Skew (przesunięcie rozkładu cech przez szum Gaussa).
\item Noisy Labels (część etykiet losowo zaburzona).
\item Label Poisoning (pełne odwrócenie klas u jednego klienta).
\item Corrupted Client (połączenie szumu cech i zatrucia etykiet).
\end{enumerate}
\end{infoboxgreen}
\end{column}
\end{columns}
...
\begin{cardpurple}[height=4.3cm]
\textbf{\footnotesize Wine Quality}\\[3pt]
\tiny \textbf{Tabularyczny (wieloklasowy).}\\[2pt]
Ocena jakości wina (skala 1--10) na podstawie cech fizykochemicznych. Rozszerza ewaluację danych tabularycznych o zadanie wieloklasowe.
\end{cardpurple}
\end{column}
\begin{column}{0.24\textwidth}
\begin{cardgreen}[height=4.3cm]
\textbf{\footnotesize Retina Dataset}\\[3pt]
\tiny \textbf{Wizyjny (Medyczny).}\\[2pt]
...
\end{column}
\begin{column}{0.24\textwidth}
\begin{cardorange}[height=4.3cm]
\textbf{\footnotesize Sentiment140}\\[3pt]
\tiny \textbf{Tekstowy (NLP).}\\[2pt]
...
\begin{column}{0.24\textwidth}
\begin{cardpurple}[height=4.3cm]
\textbf{\footnotesize CartPole (OpenGym)}\\[3pt]
\tiny \textbf{Uczenie przez wzmacnianie.}\\[2pt]
...
\toprule
\textbf{Scenariusz} & \textbf{Empirical Best} & \textbf{GA (proponowane)} & \textbf{Single-Shot LLM} & \textbf{FedAvg} & \textbf{Empirical Worst}\\
\midrule
Label Skew      & $0.6729\pm0.0149$ & \cellcolor{lightgray}$0.6679\pm0.0128$ & $0.6360\pm0.0058$ & $0.6186\pm0.0168$ & $0.6236\pm0.0205$\\
Label Poisoning & $0.6060\pm0.0168$ & \cellcolor{lightgray}$0.6223\pm0.0195$ & $0.6087\pm0.0180$ & $0.5543\pm0.0258$ & $0.5543\pm0.0258$\\
Noisy Label     & $0.6948\pm0.0187$ & \cellcolor{lightgray}$0.6882\pm0.0233$ & $0.6548\pm0.0410$ & $0.6393\pm0.0500$ & $0.6434\pm0.0471$\\
Feature Skew    & $0.7250\pm0.0139$ & \cellcolor{lightgray}$0.7112\pm0.0449$ & $0.7022\pm0.0241$ & $0.6806\pm0.0347$ & $0.6681\pm0.0429$\\
Corrupted Client& $0.6058\pm0.0258$ & \cellcolor{lightgray}$0.6120\pm0.0198$ & $0.6265\pm0.0219$ & $0.5948\pm0.0197$ & $0.5838\pm0.0230$\\
\bottomrule
\end{tabular}
\end{frame}
...
\end{column}
\begin{column}{0.5\textwidth}
\begin{infoboxgreen}{Marginalny koszt narzutu}
Wybór dokonany przez model GPT-4.1 nie generuje dodatkowego narzutu podczas właściwego treningu klientów. Cały potok jednorazowej automatyzacji LLM (detekcja + wnioskowanie) kosztuje ok. 109s wobec 69s dla samego naiwnego treningu FL -- to wciąż znacznie mniej niż 50-próbowe HPO w Optunie (1106s).
\end{infoboxgreen}
\end{column}
\end{columns}
\end{frame}
...
W uczeniu przez wzmacnianie standardowa dokładność ważona nie istnieje. Autorzy zastąpili ją \textbf{medianą średniej nagrody (reward)} z ostatnich rund treningowych, co pozwoliło na stabilną optymalizację agentów DQN.
\end{infoboxgreen}
\vspace{2pt}
\begin{infoboxorange}{Ważny limit: TinyBERT}
Przy embeddingach TinyBERT (Sentiment140) wszystkie triale Optuny dały identyczne krzywe uczenia -- silne pretrenowane reprezentacje zniwelowały różnice między klientami, czyniąc wybór strategii agregacji bez znaczenia.
\end{infoboxorange}}
\end{column}
\end{columns}
\end{frame}
...
% ============================================================
\begin{frame}{Krytyka: Ograniczona rola strategii przy silnych reprezentacjach}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Wynik testu z TinyBERT jako sygnał ostrzegawczy}\\[4pt]
Autorzy sami przetestowali architekturę typu Transformer (TinyBERT na Sentiment140) i wykazali, że przy silnych, pretrenowanych reprezentacjach wybór strategii agregacji przestaje mieć znaczenie -- wszystkie triale dały identyczne krzywe uczenia.\\[6pt]
Oznacza to, że korzyści z frameworku są \textbf{odwrotnie proporcjonalne} do jakości reprezentacji cech lokalnego modelu.
\end{column}
\begin{column}{0.5\textwidth}
\begin{infoboxorange}{Realna implikacja}
Dla najbardziej praktycznie istotnych, dużych modeli (LLM, duże Transformery z bogatymi embeddingami), potencjalna wartość dodana automatycznego doboru strategii może okazać się marginalna -- co ogranicza praktyczne zastosowanie frameworku właśnie w tych scenariuszach, gdzie FL jest dziś najbardziej pożądane.
\end{infoboxorange}
\end{column}
\end{columns}
\end{frame}
